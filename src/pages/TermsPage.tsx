  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 pb-20 pt-28">
        ...
        <p className="mt-2 text-muted-foreground">{t("terms.lastUpdated")}</p>

        <div className="mt-10 max-w-3xl space-y-8 text-sm leading-relaxed text-muted-foreground">
          {[1, 2, 3, 4, 5].map((n) => (
            <section key={n}>
              <h2 className="font-display text-lg font-bold text-foreground">{t(`terms.s${n}title`)}</h2>
              <p className="mt-2">{t(`terms.s${n}text`)}</p>
            </section>
          ))}
        </div>
      </div>
